...
UINT_PTR WINAPI InjectThread(LPVOID lpThreadParameter)
{
	WCHAR path[MAX_PATH];
	UINT_PTR pid=(UINT_PTR)lpThreadParameter;
	IthSleep(inject_delay);
	UINT_PTR s=InjectByPID(pid);
...
UINT_PTR Profile::AddThread(ThreadParam *tp)
{
	tp->hook_addr=0;
	tp->hm_index=0;
...
UINT_PTR Profile::AddLink(LinkParam* lp)
{
...
UINT_PTR Profile::AddComment(LPWSTR comment, UINT_PTR index)
{
...
void Profile::RemoveThread(UINT_PTR index)
{
	if (index!=-1&&index<thread_count)
	{
		UINT_PTR i;
...
void Profile::RemoveLink(UINT_PTR index)
{
	if (index!=-1&&index<link_count)
	{
		for (UINT_PTR i=index;i<link_count-1;i++)
			links[i]=links[i+1];
		link_count--;
...
void Profile::RemoveComment(UINT_PTR index)
{
	if (index!=-1&&index<comment_count)
	{
		delete comments[index].comment;
		for (UINT_PTR i=index;i<comment_count-1;i++)
...
public:
	...
	{
		Profile* pf=&p->data;
		UINT_PTR t=pf->hook_count*sizeof(HookParam)+sizeof(DWORD)*4;
		t+=pf->thread_count*(sizeof(ThreadParam));
...
public:
	...
	void operator() (TreeNode<LPWSTR,Profile>* p)
	{
		...
		*(DWORD*)(ptr+0x8)=pf->select_index|(pf->engine_type<<16);	
		*(DWORD*)(ptr+0xC)=((UINT_PTR)name-(UINT_PTR)buffer)&0xFFFFFFFF;
		size_t i,len;
		len=wcslen(p->key);
		wcscpy(name,p->key);
		name+=len+1;		
		...
		for (i=0;i<pf->thread_count;i++,ptr+=sizeof(ThreadParam))
			memcpy(ptr,pf->threads+i,sizeof(ThreadParam));
		...
		for (i=0;i<pf->comment_count;i++,ptr+=sizeof(CommentParam))
		{
			...
			*(DWORD*)(ptr+4)=(UINT_PTR)name-(UINT_PTR)buffer;
...
{
	HANDLE hFile=IthCreateFile(L"ITH64.pro",GENERIC_READ,FILE_SHARE_READ,FILE_OPEN);
	if (hFile==INVALID_HANDLE_VALUE) return;
	UINT_PTR i,j,profile_count;
	IO_STATUS_BLOCK ios;
	...
	for (i=0;i<profile_count;i++)
	{
		...
		ThreadParam tp={0};
		for (j=0;j<thread_count;j++,ptr+=sizeof(ThreadParam))
		{
			memcpy(&tp,ptr,sizeof(ThreadParam));
			p->AddThread(&tp);
		}
		for (j=0;j<link_count;j++,ptr+=sizeof(LinkParam))
			p->AddLink((LinkParam*)ptr);
		for (j=0;j<comment_count;j++,ptr+=sizeof(CommentParam))
			p->AddComment((LPWSTR)(buffer+*(DWORD*)(ptr+sizeof(DWORD))),*(WORD*)ptr);
	}
	delete buffer;	
}
void ProfileManager::SaveProfile()
{
	...
	*(DWORD*)(buffer)=pftree.Count();
	...
	HANDLE hFile=IthCreateFile(L"ITH64.pro",GENERIC_WRITE,FILE_SHARE_READ,FILE_OVERWRITE_IF);
	IO_STATUS_BLOCK ios;
...
void ProfileManager::RefreshProfileAddr(UINT_PTR pid, LPWSTR path)
{
...
void ProfileManager::SetProfileEngine(LPWSTR path, UINT_PTR type)
{
	ProfileNode* pfn=pftree.Search(path);
	if (pfn)
