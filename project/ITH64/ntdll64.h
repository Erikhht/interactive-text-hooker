...
{
	UINT_PTR	UniqueProcess;
	UINT_PTR	UniqueThread;
...
typedef struct _PEB
{
	...
	PVOID				Mutant;								// 8
	PVOID				ImageBaseAddress;					// 10
	PPEB_LDR_DATA		Ldr;								// 18
	PPROCESS_PARAMETERS	ProcessParameters;					// 20
	PVOID				SubSystemData;						// 28 
	PVOID				ProcessHeap;						// 30
	KSPIN_LOCK			FastPebLock;						// 38
	PVOID				AtlThunkSListPtr;					// 40
	PVOID				IFEOKey;				// 48
	UINT_PTR				EnvironmentUpdateCount;				// 50
	PVOID				*KernelCallbackTable;				// 58
	PVOID				EventLogSection;					// 60
	PVOID				EventLog;							// 68
	UINT_PTR				TlsExpansionCounter;				// 70
	PRTL_BITMAP			TlsBitmap;							// 78
	ULONG				TlsBitmapData[0x2];					// 80
	PVOID				ReadOnlySharedMemoryBase;			// 88
	PVOID				ReadOnlySharedMemoryHeap;			// 90
	PTEXT_INFO			ReadOnlyStaticServerData;			// 98
	PVOID				InitAnsiCodePageData;				// A0
	PVOID				InitOemCodePageData;				// A8
	PVOID				InitUnicodeCaseTableData;			// B0
	ULONG				KeNumberProcessors;					// B8
	ULONG				NtGlobalFlag;						// BC
	LARGE_INTEGER		MmCriticalSectionTimeout;			// C0
	UINT_PTR				MmHeapSegmentReserve;				// C8
	UINT_PTR				MmHeapSegmentCommit;				// D0
	UINT_PTR				MmHeapDeCommitTotalFreeThreshold;	// D8
	UINT_PTR				MmHeapDeCommitFreeBlockThreshold;	// E0
	ULONG				NumberOfHeaps;						// E8
	ULONG				AvailableHeaps;						// EC
	PHANDLE				ProcessHeapsListBuffer;				// F0
	PVOID				GdiSharedHandleTable;				// F8
	PVOID				ProcessStarterHelper;				// 100
	PVOID				GdiDCAttributeList;					// 108
	KSPIN_LOCK			LoaderLock;							// 110
	ULONG				NtMajorVersion;						// 118
	ULONG				NtMinorVersion;						// 11C
	USHORT				NtBuildNumber;						// 120
	USHORT				NtCSDVersion;						// 122	
	ULONG				PlatformId;							// 124
	ULONG				Subsystem;							// 128
	ULONG				MajorSubsystemVersion;				// 12C
	ULONG				MinorSubsystemVersion;				// 130
	KAFFINITY			AffinityMask;						// 138
	ULONG				GdiHandleBuffer[0x3C];				// 140
	PVOID				PostProcessInitRoutine;				// 230
	UINT_PTR				TlsExpansionBitmap;					// 238
	ULONG				TlsExpansionBitmapBits[0x20];		// 240
	UINT_PTR				SessionId;							// 2C0
	ULARGE_INTEGER		AppCompatFlags;						// 2C8
/*	PWORD				CSDVersion;							// 2D0
	PVOID				AppCompatInfo;
...
typedef struct _SERVICE_DESCRIPTOR_TABLE
{
	...
	PUINT_PTR		puCounterTable;    // array of counters
...
typedef struct _SYSTEM_PROCESS_INFORMATION {          // common members
  ...
  KPRIORITY      BasePriority;
  UINT_PTR          dUniqueProcessId;
  UINT_PTR          dInheritedFromUniqueProcessId;
  UINT_PTR          dHandleCount;
  UINT_PTR          dReserved07;
  UINT_PTR          dReserved08;
  VM_COUNTERS    VmCounters;      // see ntddk.h
  UINT_PTR          dCommitCharge;   // bytes
...
NtQuerySystemInformation(
	...
	IN UINT_PTR			uSystemInformationLength,
	OUT PUINT_PTR			puReturnLength OPTIONAL
	);
...
RtlTimeToSecondsSince1970(
	...
	OUT PUINT_PTR			SecondsSince1970
);
...
NtQuerySemaphore(
	...
	OUT PUINT_PTR ResultLength OPTIONAL
);
...
NtCreateFile(
	...
	UINT_PTR				uFileAttributes,
	UINT_PTR				uShareAccess,
	UINT_PTR				uCreateDisposition,
	UINT_PTR				uCreateOptions,
	PVOID				pEaBuffer,
	UINT_PTR				uEaLength
);
...
typedef struct _PROCESS_BASIC_INFORMATION
{
	...
	KAFFINITY	AffinityMask;
	KPRIORITY	BasePriority;
	UINT_PTR		uUniqueProcessId;
	UINT_PTR		uInheritedFromUniqueProcessId;
...
NtSuspendThread(
	...
	OUT PUINT_PTR	PreviousSuspendCount OPTIONAL
);
...
NtQueryInformationThread(
	...
	IN UINT_PTR			uThreadInformationLength,
	OUT PUINT_PTR			puReturnLength OPTIONAL
);
...
NtQuerySection(
	...
	IN UINT_PTR SectionInformationLength,
	OUT PUINT_PTR ResultLength OPTIONAL
);
...
NtQueryObject(
	...
	IN UINT_PTR ObjectInformationLength,
	OUT PUINT_PTR ReturnLength OPTIONAL
);
...
NtAllocateVirtualMemory(
	...
	IN UINT_PTR ZeroBits,
	IN OUT PUINT_PTR AllocationSize,
	IN UINT_PTR AllocationType,
	IN UINT_PTR Protect
);
...
NtQueryVirtualMemory(
	...
	IN UINT_PTR MemoryInformationLength,
	OUT PUINT_PTR ReturnLength OPTIONAL
);
...
NtProtectVirtualMemory(
	IN HANDLE ProcessHandle,
	IN OUT PVOID *BaseAddress,
	IN OUT PUINT_PTR ProtectSize,
	IN UINT_PTR NewProtect,
	OUT PUINT_PTR OldProtect
);
...
RtlEnlargedUnsignedDivide(
	...
	IN ULONG			uDivisor,
	OUT PUINT_PTR			puRemainder OPTIONAL
	);
...
NtProtectVirtualMemory(
  IN HANDLE               ProcessHandle,
  IN OUT PVOID            *BaseAddress,
  IN OUT PUINT_PTR           NumberOfBytesToProtect,
  IN UINT_PTR                NewAccessProtection,
  OUT PUINT_PTR              OldAccessProtection 
);
...
NtMapViewOfSection(
  ...
  IN HANDLE               ProcessHandle,
  IN OUT PVOID            *BaseAddress,
  IN UINT_PTR                ZeroBits OPTIONAL,
  IN UINT_PTR                CommitSize,
  IN OUT PLARGE_INTEGER   SectionOffset,
  IN OUT PUINT_PTR           ViewSize,
