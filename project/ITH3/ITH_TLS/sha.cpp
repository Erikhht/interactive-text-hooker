...
void HashMD5Final(MD5_Context* ctx, void* hash)
{
	...
	printf("MD5Final: ctx=%.8X\n",ctx);
	printf("MD5Final: hash=");
	for (int i = 0; i < HASH_SIZE_MD5; i++) printf("%.2X",h[i]);
	printf("\n");*/
}
void HashMD5Block(void* hash_block, MD5_Context* ctx)
{
	u32* block = (u32*)hash_block;
	int a,b,c,d,f,g,i,t;
	a = ctx->h0;
	b = ctx->h1;
	c = ctx->h2;
...
void HashSHA1Final(SHA1_Context* ctx, void* hash)
{
	if (ctx->remain_len >= HASH_BLOCK_SHA1 - 8)
	{
		...
		ctx->len <<= 3;
		*(u32*)(ctx->remain + HASH_BLOCK_SHA1 - 8) = BSWAP(ctx->len_high);
		*(u32*)(ctx->remain + HASH_BLOCK_SHA1 - 4) = BSWAP(ctx->len_low);
		HashSHA1Block(ctx->remain,ctx);
	}
	else
	{
	...
	/*printf("SHA1Final: ctx=%.8X\n",ctx);
	printf("SHA1Final: hash=");
	for (int i = 0; i < HASH_SIZE_SHA1; i++) printf("%.2X",hs[i]);
	printf("\n");*/
}
void HashSHA1Block(void* hash_block, SHA1_Context* ctx)
{
	unsigned int a,b,c,d,e,T,i;
	u8* block = (u8*)hash_block;
	u32 w[0x50];
	a = ctx->h0;
	b = ctx->h1;
	c = ctx->h2;
	d = ctx->h3;
	e = ctx->h4;
	for (i = 0; i < 16; i++) w[i] = BSWAP(*(u32*)(block + i * 4));
...
void HashSHA1(void* msg, u32 len, void* hash)
{
	SHA1_Context ctx;
	HashSHA1Init(&ctx);
...
void HashSHA256Final(SHA256_Context* ctx, void* hash)
{
	if (ctx->remain_len >= HASH_BLOCK_SHA256 - 8)
	{
		...
		ctx->len <<= 3;
		*(u32*)(ctx->remain + HASH_BLOCK_SHA256 - 8) = BSWAP(ctx->len_high);
		*(u32*)(ctx->remain + HASH_BLOCK_SHA256 - 4) = BSWAP(ctx->len_low);
		HashSHA256Block(ctx->remain,ctx);
	}
	else
	{
...
void HashSHA256(void* msg, u32 len, void* hash)
{
	SHA256_Context ctx;
	HashSHA256Init(&ctx);
